...
- 抽出配列座標系（1..L）に再マッピングした annotation を出力（mVISTA簡易テキスト形式 + GFF3/BED）
...
- `{species}.mvista.annotation.txt`: mVISTA簡易annotation形式（`> / <` gene行 + `exon/utr` 行）
...
- mVISTAのannotation入力には、元の全ゲノムGFF3ではなく `*.mvista.annotation.txt`（必要に応じて`.gff3`/`.bed`）を使ってください
